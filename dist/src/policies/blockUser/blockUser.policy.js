...
var blockUserPolicy = {
  body: Joi.object().keys({
    ...
    iat: Joi.any(),
    exp: Joi.any(),
    blockUserId: Joi.number().required().messages(_objectSpread(_objectSpread({}, combineValidationMessages(['any.required', 'number.empty'], generateMandatoryFieldMessage('blockUserId'))), {}, {
      'number.base': 'blockUserId should be of type number'
    })),
    action: Joi.string().required().messages(_objectSpread(_objectSpread({}, combineValidationMessages(['any.required', 'string.empty'], generateMandatoryFieldMessage('action'))), {}, {
